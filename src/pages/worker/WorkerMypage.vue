<template>
  ...
  <div class="max-w-[728px] w-full mx-auto">
    ...
    <!-- 달력 그리드 -->
    <div
      class="grid grid-cols-7 border border-[#D9D9D9] rounded-b-lg overflow-hidden">
      <div
  ...
  <!-- 근무 현황 -->
  <div
    class="w-full max-w-[730px] mx-auto mt-[45px] mb-[140px] p-4 bg-white rounded-md transition-all duration-300">
    ...
    <div class="flex flex-wrap justify-between gap-4 items-start">
      ...
      <div
        ...
        <div
          class="text-[#505050] text-base font-medium font-['Pretendard'] mb-1">
          출근
        </div>
      ...
      <div
        ...
        <div class="text-[#767676] text-lg font-medium font-['Pretendard']">
          0
        </div>
      </div>
      <!-- 지각 -->
      ...
      <!-- 인센티브 -->
      <div
        class="flex flex-col items-center justify-center bg-white rounded-[10px] pt-6">
        <div
          class="text-[#505050] text-base font-medium font-['Pretendard'] mb-1 text-center">
          이 달의 인센티브
        </div>
        <div
    ...
    <div v-if="attendanceCount === 0" class="text-center py-8 mt-6">
      <div class="text-[#767676] text-base">
        {{ currentPage.month }}월에는 근무 기록이 없습니다.
      </div>
    </div>
  </div>
  ...
  <div
    ...
    <router-link to="/worker/worker-home">
      <div
        ...
        <div
          class="w-12 text-center text-[#111] text-base font-medium font-['Pretendard']">
          홈
        </div>
      </div></router-link
    >
    ...
    <!-- 알림 아이콘 -->
    <div
      class="flex flex-col items-center gap-2 w-14 cursor-pointer transition-transform hover:scale-105">
    ...
    <router-link to="/worker/worker-mypage">
      <div
        ...
        <div
          class="w-20 text-center text-[#111] text-base font-medium font-['Pretendard']">
          마이페이지
        </div>
      </div></router-link
    >
    ...
    <!-- 환경설정 아이콘 -->
    <div
      class="flex flex-col items-center gap-2.5 w-14 cursor-pointer transition-transform hover:scale-105">
      <img src="/images/kang/settings.png" alt="settings" />
      <div
        class="w-full text-center text-[#111] text-base font-medium font-['Pretendard']">
        환경설정
      </div>
    </div>
  </div>
...
const workData = ref({
  ...
  //3월 데이터
  "2025-03-02": { shift: "오전", pay: 28500 },
  "2025-03-03": { shift: "오전", pay: 31200 },
  "2025-03-04": { shift: "오후", pay: 29800 },
  "2025-03-05": { shift: "오후", pay: 33500 },
  "2025-03-09": { shift: "오전", pay: 27600 },
  "2025-03-10": { shift: "오전", pay: 32100 },
  "2025-03-11": { shift: "오후", pay: 30400 },
  "2025-03-12": { shift: "오후", pay: 35200 },
  "2025-03-16": { shift: "오전", pay: 26800 },
  "2025-03-28": { shift: "오전", pay: 29900 },
  "2025-03-29": { shift: "오후", pay: 31750 },
  "2025-03-30": { shift: "오전", pay: 24600 },
});
...
const currentMonthLabel = computed(() => {
  return `${currentPage.value.year}년 ${String(
    currentPage.value.month
  ).padStart(2, "0")}월 근무 현황`;
});
...
const calendarDates = computed(() => {
  ...
  while (current <= endDate) {
    dates.push({
      ...
      inMonth:
        current.getMonth() + 1 === month && current.getFullYear() === year,
    });
...
function getShiftClass(shift) {
  return shift === "오전"
    ? "bg-[#45A6FF]/20 text-[#111]"
    : "bg-[#5FC95E]/20 text-[#111]";
}
...
const attendanceCount = computed(() => {
  ...
  return Object.keys(workData.value).filter((key) =>
